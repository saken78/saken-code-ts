...
export class PromptRegistry {
  ...
  /**
   * Returns an array of prompts by type (e.g., 'claude-prompt', 'mcp-tool', etc.)
   */
  getPromptsByType(type) {
    const typePrompts = [];
    for (const prompt of this.prompts.values()) {
      if (prompt.type === type) {
        typePrompts.push(prompt);
      }
    }
    return typePrompts.sort((a, b) => a.name.localeCompare(b.name));
  }
  ...
  /**
   * Removes all prompts of a specific type.
   */
  removePromptsByType(type) {
    for (const [name, prompt] of this.prompts.entries()) {
      if (prompt.type === type) {
        this.prompts.delete(name);
      }
